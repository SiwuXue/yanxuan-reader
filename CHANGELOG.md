...
- Web 界面「关于」从设置抽屉移到主页面右栏（"我的书架"下方），独立卡片展示：
  版本号、项目介绍与项目仓库（GitHub）、作者博客、爱发电赞助三个外链，一眼可见，
  不再藏在设置里。
...
- 修复「导出的 EPUB 没有正文」：登录态失效时知乎返回的是 **HTTP 200 的登录/人机验证页**
  （不是 403），旧实现把这个只含几张站点 logo、没有任何文字的空壳页解析成正文并存进断点，
  于是导出成一本空白书；更糟的是断点把它记成「本章已完成」，重新登录后重跑只会命中缓存、
  把空白书重新导出一次，永远好不了。现在三层防线一起兜住：
  - `parse/parser.py` 新增 `detect_gate_page()`（按 `/account/unhuman`、站点默认标题
    「知乎 - 有问题，就会有答案」「安全验证 - 知乎」、以及「无 og:title 且无正文容器」三重判据），
    `parse_article` 遇到「只有图片、没有任何文字且连正文容器都没找到」一律抛 `ParseError`，
    错误消息直接说明「重新登录后用同一链接重新下载即可」；真·图片类章节
    （有 `div.RichText`）不会误杀。
  - `engine/checkpoint.py` 的「已完成」判定从「存在且可解析」升级为「存在且**有正文**」
    （`Article.has_body_text()`），空壳缓存一律视为未完成、续传时自动重取 —— 已有的坏缓存
    无需手动 `--no-resume` 也能自愈。
  - `engine/fetcher.py` 在导出前再校验一次缓存，空壳视同缓存缺失走现场重取；
    专栏目录页解不出章节时也用 `detect_gate_page` 区分「链接给错了」与「登录态掉了」，
    不再笼统地让用户去排查链接。
- `types.Article.has_body_text()`：把「正文有效」的口径收敛为唯一定义，供解析层、断点层共用。

### Added

- Web 界面「设置 → 关于」补全项目信息：新增一句话项目介绍，以及项目仓库
  （GitHub）、作者博客、爱发电赞助三个外链，均为浏览器新标签页打开。


## [5.0.1] - 2026-09-14

- 修复盐选字体反爬：相同 Unicode 字形名改用 OCR 识别，按正文实际字体分别还原字符。
- 保留 HTML 属性与脚本，避免不同字体的映射互相污染；增大字形渲染尺寸并缓存 OCR 结果。
- 发布包内嵌 OCR 模型、ONNX 运行库与字体依赖，支持 Windows x64、Linux x64、macOS arm64。
- 更新检查和项目链接迁移至 SiwuXue/yanxuan-reader。
- 已下载的旧乱码文件需要取消“续传”重新下载；升级不会自动改写旧文件。
